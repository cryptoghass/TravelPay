...
*Setup instructions for a gitian build of Travel Pay using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Travel Pay
...
- [Building Travel Pay](#building-travelpay)
...
- The VM will detect network settings using TRPP, this should all proceed automatically
...
Clone the git repositories for travelpay and gitian and then checkout the travelpay version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/travelpay-crypto/travelpay.git
cd travelpay
...
Follow the instructions in [doc/release-process.md](release-process.md) in the travelpay repository
...
To build Travel Pay (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the travelpay repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/travelpay/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/travelpay-crypto/travelpay
...
For example:
```bash
URL=https://github.com/crowning-/travelpay.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit travelpay=${COMMIT} --url travelpay=${URL} ../travelpay/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit travelpay=${COMMIT} --url travelpay=${URL} ../travelpay/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit travelpay=${COMMIT} --url travelpay=${URL} ../travelpay/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/travelpay-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/travelpay-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/travelpay-build.assert
```
...
[travelpay/gitian.sigs](https://github.com/travelpay-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
