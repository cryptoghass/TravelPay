...
#if defined(NDEBUG)
#error "Travel Pay cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight <= 4 && nHeight > 0) {
        nSubsidy = 0 * COIN;
    } else if (nHeight <= 5 && nHeight > 4) {
        nSubsidy = 25001 * COIN;
    } else if (nHeight <= 200 && nHeight > 5) {
        nSubsidy = 0 * COIN;
    } else if (nHeight <= 75000 && nHeight > 200) {
        nSubsidy = 100 * COIN;
	} else if (nHeight <= 895400 && nHeight > 75000) {
		nSubsidy = 50 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
{
    RenameThread("travelpay-scriptch");
